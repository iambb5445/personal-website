...
Modeling procedural content generation as a constraint satisfaction problem makes it possible to enforce hard local and global constraints, such as reachability, on the generated output. But a generator's perceived quality often depends on [statistical properties](/projects/1-context-sensitive-wfc) too, and such properties can't be expressed directly as a hard constraint on any single output. Methods that skip the general-purpose solver, like Gumin's original WaveFunctionCollapse (WFC), can shape these statistics but have limited constraint propagation and can't express non-local constraints.
